...
class IconListModel(QStandardItemModel):
	...
	def borderIcon (self, file, size, containsInDB, measured=None):
...
class MainWindow(QMainWindow):
	...
	def updateDB(self, searchInFiles, progress_callback):
		...
		for i in range(self.dirModel.totalFileCount):
			...
			#if len(assetsList) > 0:
			self.db.insert({'thumb': file, 'assets': assetsList, 'assetsRes': assetsResList, 'webSource': webSource, 'assetDims': assetDims})
