...
class MainWindow(QMainWindow):
	...
	def updateThumbsDB(self, assetsDict, **kwargs):
		...
		for i in range(self.dirModel.totalFileCount):
			...
			base64Image = self.filesmodel.borderIcon(file, self.listView.iconSize(), purchased, measured, True)
			#print(file)
	...
	def updateAssetsDB(self, assetsDict, **kwargs):
		...
		for count, (key, value) in enumerate(assetsDict.items()):
			...
			if existingItem:
				newItemIcon = self.filesmodel.updateIconFromBase64(existingItem['image'])
	...
	def createItemIcons(self):
		try:
			...
			for count, item in enumerate(batch):
				...
				#worker = Worker(self.loadItemIcon, None, smallThumb, self.listView.iconSize(), qMIndex, self.filesmodel, parsedItem['containsInDB'])
				worker = Worker(self.loadItemIcon, None, qMIndex, self.filesmodel, item['image'])
				#worker = Worker(self.loadItemIcon, None, item, self.listView.iconSize(), qMIndex, self.filesmodel)
