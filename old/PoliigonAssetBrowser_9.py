...
#incoming list is a full path so no need to add to basepath, 
#maybe no need for basepath at all
def createMSFile(basepath, fileResList, file2DList, dimensions, materialName):
	
	print(dimensions)

	def parseDimesions(dim):
		print(dim)
	...
	poliiMat = poliigonMaterialSet _2K_:new1 _3K_:new2 _4K_:new3 _6K_:new4 _HIRES_:new5
	'''
...
#using subSourced models
class CustomProxyModel2(QSortFilterProxyModel):
	def __init__(self,_sourceModel):
		super().__init__()
		self._sourceModel = _sourceModel
		self.nameModel = QSortFilterProxyModel(self)
		self.nameModel.setSourceModel(self._sourceModel)
		self.nameModel.setFilterCaseSensitivity(False)

		self.roleModel = QSortFilterProxyModel(self)
		self.roleModel.setSourceModel(self.nameModel)
		self.roleModel.setFilterCaseSensitivity(False)
		self.setSourceModel(self.roleModel)

	def mapToSourceDeep(self, index):
		indexRole = self.mapToSource(index)
		indexName = self.roleModel.mapToSource(indexRole)
		return self.nameModel.mapToSource(indexName)

	def sourceModelDeep(self):
		return self._sourceModel

	def setNameFilter(self, text):
		self.nameModel.setFilterRole(Qt.DisplayRole)
		self.nameModel.setFilterRegExp(text)
	
	def setRoleFilter(self, text):
		self.roleModel.setFilterRole(Qt.UserRole + 4)
		self.roleModel.setFilterRegExp(text)
...
class MainWindow(QMainWindow):
	...
	def on_listView_clicked(self, index):	
		index = self.sender().model().mapToSource(index)
		self.sender().model().sourceModel().setDimensions(index)
		print('set dims')
	
	@pyqtSlot(QModelIndex)
	def on_listView_doubleClicked(self, index):
		index = self.sender().model().mapToSource(index)
		self.sender().model().sourceModel().exploreAsset(index)
	...
	def checkIfContains(self, file, db, query):
		...
		if db.contains(query.thumb == file):
			...
			#get highest res from a list of res, string comparisson work in this case hires > 6k > 3k
