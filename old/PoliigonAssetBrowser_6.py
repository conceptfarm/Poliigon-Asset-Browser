...
class IconListItem(QStandardItem):
	...
	def borderIcon (self, input, borderColour, borderWidth):
	...
	def setThumbs(self, tup):
		...
		if self.containsInDB:
			transImg = self.borderIcon(img, Qt.green, 2)
			icon = QIcon((transImg))
			self.setPurchased(True)
		else:
			transImg = self.borderIcon(img, Qt.gray, 0)
			icon = QIcon((transImg))
...
class IconListView(QListView):
	...
	def startDrag(self, e):
		...
		if thisItem.purchased:
			...
			itemName = (((PurePath(thisItem.thumbPath).stem).replace(' ',''))[:-3])
			tempMSFile = createMSFile(thisItem.assetBaseDir, thisItem.assetFiles, thisItem.dimensions, itemName)
...
class IconListModel(QStandardItemModel):
	...
	def data(self, index, role):
		...
		if role == Qt.DisplayRole:
			#return QVariant(self.filesList[index])
			#newItem = QStandardItem(self.filesList[index])
			#self.setItem(1,newItem)
			return self.itemFromIndex(self.index(index,0)) #os.path.basename(fileList[i])
			#print (self.filesList[index])
			#return self.it.text(self.filesList[index])
	...
	def setAllData(self):
		for i in range(len(self.filesList)):
			#print(self.filesList[i])
			self.setData(self.index(i,0), self.filesList[i], Qt.DisplayRole)

	def setData(self, index, value, role):

		#if role == Qt.DisplayRole:
			#self.it.text(os.path.basename(value))
...
class MainWindow(QMainWindow):
	...
	def setupUi(self, MainWindow):
		...
		self.filesmodel2 = IconListModel()
		self.prt = IconListItem(self.listView.iconSize(), self.listView.gridSize(), '', False)
		self.filesmodel2.setItemPrototype(self.prt )

		self.listView2 = IconListView()
		self.listView2.setModel(self.filesmodel2)
		self.listView2.setObjectName('listView2')
		self.listView2.setViewMode(1)
		self.listView2.setResizeMode(1)
		self.listView2.setWrapping(True)
		self.listView2.setWordWrap(True)
		self.listView2.setUniformItemSizes(True)
		self.listView2.setIconSize(self.listView2.gridSize()*0.85)
		...
		self.splitterLayout.addWidget(self.listView2)
	...
	def on_treeView_clicked(self,index):
		...
		self.filesmodel2.filesList = fileList
		self.filesmodel2.setAllData()
		print(self.filesmodel2.data(2,Qt.DisplayRole))
		#print(self.filesmodel2.filesList)
