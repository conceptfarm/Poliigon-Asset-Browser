...
class MainWindow(QMainWindow):
	...
	def __init__(self,poliigonThumbPath,poliigonMapsPath):
		...
		self.dbAssetsPath = os.path.join(self.poliigonThumbPath, 'poliigonAssets.json')
	...
	def on_actionReBuildDB_triggered(self, doUpdateDB=False):
		...
		if doUpdateDB:
			...
			worker.signals.progressInt.connect(self.progressWindow.setValue)
			worker.signals.result.connect(self.setSomeResult)
			worker.signals.finished.connect(self.endProgressUpdateDBWindowAssets)
			self.threadpool.start(worker)
	
	def setSomeResult(self, res):
		self.someresult = res
		print(self.someresult)
	...
	def on_actionReBuildDBAssets_triggered(self, doUpdateDB=False):
		...
		if doUpdateDB:
			...
			self.progressWindow.setMinimum(0)
			self.progressWindow.setMaximum(len(allFiles))
			
			self.threadpool.setMaxThreadCount(1)
			worker = Worker(self.updateAssetDB,'int', allFiles)
			worker.signals.progressInt.connect(self.progressWindow.setValue)
			worker.signals.finished.connect(self.endProgressUpdateDBWindowAssets)
			self.threadpool.start(worker)	
		'''
	...
	def updateThumbsDB(self, assetsDict, **kwargs):
		...
		progress_setmax.emit(self.dirModel.totalFileCount)
		...
		for i in range(self.dirModel.totalFileCount):
			...
			pickledImage = self.filesmodel.borderIcon(file, self.listView.iconSize(), False, measured, True)
			#print(file)
	...
	def updateDB_old(self, searchInFiles, progress_callback):
		#for each thumbnail in the model
		#for i in range(5):
		for i in range(self.dirModel.totalFileCount):
			file = self.dirModel.allThumbFiles[i]
	...
	def updateAssetDB(self, searchInFiles, progress_callback):

		self.dbAssets.close()
		self.dbAssets = TinyDB(storage=CachingMiddleware(MemoryStorage))

		def resFromFilePath(file):
			res = ''
			#create list by spliting the file name ie ['FenceChainLink001', 'COL', '6K.png']
			assetRes = os.path.basename(file).split('_')
			if len(assetRes) > 1:
				#if last element is ex. "metalness.jpg" then get the second to last element
				#which should be the res designation
				if (assetRes[-1].split('.'))[0].lower() == 'metalness':
		...
		i = 0
		count = 0
		while i-1 < len(searchInFiles)-1:
			#first file in a purchased directory
			thisFile = searchInFiles[i]
			
			#Get filename from path without ext, remove spaces and remove the '_tn' from the end
			searchName = (os.path.basename(thisFile).split('_'))[0]		

			assetsList = [os.path.dirname(os.path.abspath(thisFile))]
			assetsResList = [resFromFilePath(thisFile)]

			j = len(searchInFiles) - 1
			m = i 
			while j > m:
				#split out just the file name up to the first _
				if searchName == (os.path.basename(searchInFiles[j]).split('_'))[0]:
					#append just the directory path of the file ie Y:\Maps\Poliigon_com\Fence_Mesh_Grate\FenceChainLink001_6k
					assetsList.insert(1, os.path.dirname(os.path.abspath(searchInFiles[j])))
					assetsResList.insert(1,resFromFilePath(searchInFiles[j]))
					del searchInFiles[j]
					count = count + 1
				j = j - 1
			
			if len(assetsResList) < 2:
				print (assetsList)
			self.dbAssets.insert({'name': searchName, 'assets': assetsList, 'assetsRes': assetsResList})
			
			count = count + 1
			i = i  + 1
			
			#need this here, update every 5th otherwise buffer overrun error
			if count % 5 == 0:
				try:
	...
	def updateAssetsDB(self, assetsDict, **kwargs):
		...
		#not working because the db is deleted before the worker is launched
		for count, (key, value) in enumerate(assetsDict.items()):
			assetsList = value['assets']
			assetsResList = value['assetsRes']
			#searchResult = self.db.search(self.dbQuery.name.search(filterName, flags=re.IGNORECASE))
			searchResult = self.db.search(self.dbQuery.name.test(dbTestFunc, key))
			print(searchResult)
	...
	def collectPurchasedFiles(self, **kwargs):
		...
		while it.hasNext():
			thisDir = it.next()
			total = len(os.listdir(thisDir))
			
			#progress_setmin.emit(0)
			#if total> updateEvery:
				#progress_setmax.emit(total)
	...
	def createItemImage(self, i, thisModel, _bytes, **kwargs):
