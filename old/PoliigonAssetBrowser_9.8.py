...
class IconListModel(QStandardItemModel):
	...
	def exploreAsset(self, index):
		if self.data(index,Qt.UserRole+4): #if purchased go to file
			path = self.data(index,Qt.UserRole+3)
			os.startfile(path)
		else:
			try:
				webSource = self.data(index,Qt.UserRole+5)
				if webSource !=None:
					webbrowser.open_new(webSource)
				'''
				#not sure why this was like this before?
...
class MainWindow(QMainWindow):
	...
	def updateThumbsDB(self, assetsDict, **kwargs):
		...
		for i in range(self.fileModel.totalFileCount):
			...
			jsonFile = thumbFileName[:-3] + '.json' #remove _tn and add .json file ext
			assetsList = []
			assetsResList = []
