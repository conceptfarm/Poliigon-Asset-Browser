...
class IconListView(QListView):
	...
	def startDrag(self, e):
		...
		#jsonFile = self.model().data(self.currentIndex() , Qt.UserRole + 1)
...
class IconListModel(QStandardItemModel):
	...
	def borderIcon (self, base64String, size, containsInDB, measured=None, asBytes=False):
		bgImage = QPixmap(size)
		#bgImage.fill(Qt.transparent)
		bgImage.fill(palette.baseColour)

		#originalImage = QImage(file)
		originalImage = imageCollectionBase64.qImageFromBase64String(self, base64String)
...
class MainWindow(QMainWindow):
	...
	def updateThumbsDB(self, assetsDict, **kwargs):
		...
		for i in range(len(self.fileModel.allThumbFiles)):
			...
			base64Image = self.iconListModel.borderIcon(base64ImageString, self.iconListView.iconSize(), purchased, measured, True)
